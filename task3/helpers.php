...
function asset($relativePath) { return BASE_URL . ltrim($relativePath, "/"); }
function redirect($page, $params = []) { header("Location: " . url($page, $params)); exit; }
function json_response($data, $httpCode = 200) {
    http_response_code($httpCode); header("Content-Type: application/json; charset=utf-8"); echo json_encode($data); exit;
}
...
function save_uploaded_image($fileField, $targetDir, &$error) {
    ...
    $ext = $ALLOWED_IMAGE_MIME[$mime]; $filename = uniqid("img_", true) . "." . $ext;
    if (!move_uploaded_file($file["tmp_name"], $targetDir . "/" . $filename)) { $error = "Could not save the uploaded image."; return false; }
    return $filename;
}
